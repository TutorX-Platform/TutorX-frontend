                ...
                this.dummyService.pay(this.stripeData).subscribe(
                  (res) => {
                    ...
                    if (res['status'] === 200) {
                      ...
                      // this.dialogRef.close();
                      // @ts-ignore
                      this.utilService.openDialog(systemMessage.questionTitles.paymentFailed, res["error"], constants.messageTypes.warningInfo).afterOpened().subscribe();
                      console.log(res);
                    }
                  }, () => {
                    progressDialog.close();
                    // @ts-ignore
                    this.utilService.openDialog(systemMessage.questionTitles.paymentFailed, "Payment failed Due to Unknown reason Please check your card number", constants.messageTypes.warningInfo).afterOpened().subscribe()
                  }
                )
              } else {
                // this.dialogRef.close();
